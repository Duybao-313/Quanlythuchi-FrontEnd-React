      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
        ...
        <div className="group relative bg-gradient-to-br from-indigo-500 via-indigo-600 to-purple-600 rounded-2xl p-6 text-white shadow-xl shadow-indigo-500/20 hover:shadow-indigo-500/40 transition-all duration-300 overflow-hidden">
          ...
          <div className="relative">
            ...
            <p className="text-3xl font-bold">{totalBalance.toLocaleString()}<span className="text-lg ml-1">₫</span></p>
            <p className="text-sm text-indigo-200 mt-2">Từ {wallets.length} ví</p>
          </div>
        </div>
        ...
        <div className="group relative bg-gradient-to-br from-violet-500 via-purple-500 to-fuchsia-600 rounded-2xl p-6 text-white shadow-xl shadow-violet-500/20 hover:shadow-violet-500/40 transition-all duration-300 overflow-hidden">
          ...
          <div className="relative">
            <div className="flex items-center justify-between mb-4">
              <p className="text-violet-100 text-sm font-medium">Cân bằng ròng</p>
              <div className="w-10 h-10 bg-white/20 backdrop-blur-sm rounded-xl flex items-center justify-center">
                <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
          ...
          <div className="bg-white/80 backdrop-blur-sm rounded-2xl p-6 shadow-xl border border-gray-100">
            ...
            <div className="space-y-3">
              <div className="flex items-center justify-between p-4 bg-gradient-to-r from-rose-50 to-pink-50 rounded-xl border border-rose-100">
                ...
                <span className="font-bold text-rose-600">{todayExpense.toLocaleString()}₫</span>
              </div>
              ...
              <div className="flex items-center justify-between p-4 bg-gradient-to-r from-emerald-50 to-green-50 rounded-xl border border-emerald-100">
                ...
                <span className="font-bold text-emerald-600">{todayIncome.toLocaleString()}₫</span>
              </div>
              ...
              <div className="flex items-center justify-between p-4 bg-gradient-to-r from-indigo-50 to-purple-50 rounded-xl border border-indigo-100">
                <div className="flex items-center gap-3">
                  ...
                  <span className="text-sm font-medium text-gray-700">Cân bằng</span>
                </div>
                <span className={`font-bold ${todayIncome - todayExpense >= 0 ? 'text-emerald-600' : 'text-rose-600'}`}>
          ...
          <div className="bg-white/80 backdrop-blur-sm rounded-2xl p-6 shadow-xl border border-gray-100">
            <div className="flex items-center justify-between mb-5">
              <div className="flex items-center gap-3">
                ...
                <h3 className="text-lg font-semibold text-gray-800">Ví của bạn</h3>
              </div>
              <span className="text-sm font-medium text-indigo-600 bg-indigo-50 px-2.5 py-1 rounded-lg">{walletCount}</span>
            </div>
            <div className="space-y-3">
              {wallets.slice(0, 5).map((w) => (
                <div key={w.id} className="flex items-center justify-between p-3 hover:bg-gray-50 rounded-xl transition-colors">
                  <div className="flex items-center gap-3">
                  ...
                  <span className="font-semibold text-gray-800">{w.balance.toLocaleString()}₫</span>
                </div>
              ))}
              {wallets.length > 5 && (
                <div className="text-center pt-2">
                  <span className="text-xs text-gray-500 bg-gray-100 px-3 py-1 rounded-full">+{wallets.length - 5} ví khác</span>
                </div>
              )}
            </div>
          </div>
        ...
        <div className="lg:col-span-2 bg-white/80 backdrop-blur-sm rounded-2xl p-6 shadow-xl border border-gray-100">
          <div className="flex items-center justify-between mb-6">
            ...
            {recentTransactions.length > 0 && (
              <span className="text-xs font-medium text-gray-500 bg-gray-100 px-3 py-1.5 rounded-lg">Mới nhất</span>
            )}
          </div>
          ...
          {recentTransactions.length === 0 ? (
            <div className="text-center py-12">
              ...
              <p className="text-gray-400 text-sm mt-1">Bắt đầu ghi nhận chi tiêu của bạn</p>
            </div>
          ) : (
                  ...
                  <div className="flex items-center gap-4 flex-1 min-w-0">
                    ...
                    <div className="min-w-0 flex-1">
                      ...
                      <p className="text-xs text-gray-500 mt-0.5">
                        {t.date ? new Date(t.date).toLocaleDateString("vi-VN", { weekday: 'short', day: 'numeric', month: 'short' }) : "Không rõ"}
                      </p>
                    </div>
                  </div>
