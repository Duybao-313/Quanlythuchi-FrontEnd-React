    ...
    return (
      <div className="space-y-3">
        {[...Array(3)].map((_, i) => (
          <div
            key={i}
            className="flex items-center gap-4 p-4 bg-white/80 backdrop-blur-sm rounded-xl border border-gray-100 animate-pulse"
          >
    ...
    return (
      <div className="flex flex-col items-center justify-center py-12 text-center">
        ...
        <p className="text-sm text-gray-400 mt-1">
          Hãy tạo ví để bắt đầu quản lý chi tiêu
        </p>
      </div>
    );
  }
...
function WalletItem({
  ...
  return (
    <div className="group flex items-center gap-4 p-4 bg-white/80 backdrop-blur-sm rounded-xl border border-gray-100 hover:shadow-md hover:border-gray-200 transition-all duration-200">
      <div
        className={`w-12 h-12 flex items-center justify-center rounded-xl overflow-hidden ${typeColor}`}
      >
          ...
          <span
            className={`text-xs px-2 py-0.5 rounded-full font-medium ${
              wallet.type === WalletType.CASH
                ...
                : wallet.type === WalletType.E_WALLET
